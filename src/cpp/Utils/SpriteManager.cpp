...
SpriteManager::SpriteManager(entt::registry& registry, entt::entity entity)
  {
    ...
    if (!registry.all_of<Transform, Sprite>(this->entity)) {
      throw std::logic_error(this->_className + " requires the following components from entity to Update: Sprite, and Transform");
    }
...
void SpriteManager::Update(entt::registry& registry) {
  ...
  // animated
  if (registry.all_of<AnimationMap>(this->entity) && strcmp(this->key,"") != 0) {
...
void SpriteManager::Render(entt::registry& registry)
{
  if (!registry.all_of<Sprite>(this->entity)) {
    throw std::logic_error(this->_className + " requires the following components from entity to Draw: Sprite, and Transform");
  }

  Sprite& sprite = registry.get<Sprite>(this->entity);
  TextureManager::Draw(sprite.texture, sprite.srcRect, sprite.destRect, this->spriteFlip);
}

void SpriteManager::Play(entt::registry& registry, const char* key)
{
  if (registry.all_of<AnimationMap>(this->entity) && (registry.get<AnimationMap>(this->entity)).animations.count(key)) {
    this->key = key;
  } else {
    std::invalid_argument(std::string(key) + " does not exist for entity's AnimationMap component");
  }
}

void SpriteManager::AddAnimation(entt::registry& registry, const char* key, Animation animation) {
